  ...
  return (
    <section className="my-10 px-5 w-[90%] mx-auto">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-3">
        <div>
          <h2 className="text-3xl font-volkhovBold dark:text-white">Latest News</h2>
          <p className="text-gray-600 mt-2 max-w-md font-mulishSemiBold dark:text-gray-300">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {newsArticles.slice(0, 4).map((article) => (
          ...
          >
            ...
            />
            <div className="p-4">
              <p className="text-sm text-gray-600 dark:text-white">{article.author}</p>
              <h3 className="text-lg font-bold mt-2 dark:text-gray-300">{article.title}</h3>
            </div>
          </div>
        ))}
      </div>
